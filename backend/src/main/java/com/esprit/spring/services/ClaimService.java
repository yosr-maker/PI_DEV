...
public class ClaimService implements ClaimServiceI {
	...
	public Claim save(Claim cl, long clientId, long productId) {
		...
		Product p = productRepository.findById((int) productId).get();
		cl.setClient(c);
		cl.setProduct(p);
		Date dateSys = new Date(System.currentTimeMillis());
		cl.setDateClaim(dateSys);
		cl.setDecision(ClaimDecision.PENDING);
		if (claimRepository.findByClientAndProduct(c,p) == null) {
	...
	public List<String> displayBadProductsByClaims() {
		...
		for (int i = 0; i < 3; i++) {
			...
			s = (i + 1) + " - Product: " + productRepository.findById((int) ind).get().getName() + " with " + max
					+ " claims ";
			list.add(s);
